...
export default function Profile() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        await api.post('avatares/', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
        fetchUser(); // Reload profile visually
    } catch (err) {
        alert("Error subiendo el avatar. Verifica que sea una imagen pálida.");
    }
  };

  const handleDeleteAvatar = async () => {
    try {
        const avRes = await api.get('avatares/');
        if (avRes.data.length > 0) {
            await api.delete(`avatares/${avRes.data[0].id}/`);
